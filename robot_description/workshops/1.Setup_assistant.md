...
- In the next step you will define your planning group for which MoveIt! later on tries to compute a path.
    ...
    - Choose RTTConnect as default planner.
    - Click on the **Add Kin. Chain** button and choose `base_link` as Base Link and `tool_tip` as Tip Link (see Figure 5.1).
...
- Choose the **Moveit Controllers** pane on the left
    ...
    - Change the name to ``joint_trajectory_controller`` and click save (Figure 11.1, Figure 11.2)
...
---
# 3. Adding missing configuration files
