...
> **Hint**: You can use the ROS plugin of VS Code to visualize the URDF from the start. To do so press ``Ctrl+Shift+P`` and type ``ROS: Preview URDF``.
...
* Now to add the necessary arguments for instantiating the robot, take a look at the [``ur_macro.xacro``](https://github.com/UniversalRobots/Universal_Robots_ROS2_Description/blob/ros2/urdf/ur_macro.xacro) file. You will see that the ``ur_robot``-macro can the following parameters:
    ...
    > **Note:** The ``*`` in front of the ``origin`` parameter means that the parameter is a block parameter which goes the brackets of the ``<xacro::ur_robot></xacro::ur_robot>`` tag.
